...
Scheduled.handleExpired = function () {
    return __awaiter(this, void 0, void 0, function* () {
        const now = Date.now();
        const tids = yield database_1.default.getSortedSetRangeByScore('topics:scheduled', 0, -1, '-inf', now);
        if (!tids.length) {
            return;
        }
        ...
        // @ts-ignore
        sendNotifications(uids, topicsData), updateUserLastposttimes(uids, topicsData), ...topicsData.map((topicData) => unpin(topicData.tid, topicData)), database_1.default.sortedSetsRemoveRangeByScore([`topics:scheduled`], '-inf', now)));
    });
};
...
Scheduled.pin = function (tid, topicData) {
    return __awaiter(this, void 0, void 0, function* () {
        return Promise.all([
            ...
            database_1.default.sortedSetAdd(`cid:${topicData.cid}:tids:pinned`, Date.now(), tid),
...
function unpin(tid, topicData) {
    return [
        ...
        database_1.default.sortedSetRemove(`cid:${topicData.cid}:tids:pinned`, tid),
...
function shiftPostTimes(tid, timestamp) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return database_1.default.setObjectBulk(pids.map((pid, idx) => [`post:${pid}`, { timestamp: timestamp + idx + 1 }]));
    });
}
