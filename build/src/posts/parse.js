...
let sanitizeConfig = {
    ...
    allowedClasses: Object.assign({}, sanitize.defaults.allowedClasses),
};
function default_1(Posts) {
        ...
        while (current !== null) {
            if (current[1]) {
                try {
                    ...
                    winston_1.default.verbose(err.messsage);
                }
            }
    ...
    function sanitizeSignature(signature) {
        ...
        const tagsToStrip = [];
        if (meta_1.default.config['signatures:disableLinks']) {
            tagsToStrip.push('a');
        }
        if (meta_1.default.config['signatures:disableImages']) {
