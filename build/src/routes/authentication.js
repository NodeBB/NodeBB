...
Auth.verifyToken = function (token, done) {
    return __awaiter(this, void 0, void 0, function* () {
        const { tokens = [] } = yield meta_1.default.settings.get('core.api');
...
Auth.reloadRoutes = function (params) {
    return __awaiter(this, void 0, void 0, function* () {
        loginStrategies.length = 0;
        const { router } = params;
        // Local Logins
        if (plugins.hooks.hasListeners('action:auth.overrideLogin')) {
            winston_1.default.warn('[authentication] Login override detected, skipping local login strategy.');
        ...
        try {
            loginStrategies = yield plugins.hooks.fire('filter:auth.init', loginStrategies);
        }
        catch (err) {
            winston_1.default.error(`[authentication] ${err.stack}`);
        }
        loginStrategies = loginStrategies || [];
        loginStrategies.forEach((strategy) => {
            if (strategy.url) {
                router[strategy.urlMethod || 'get'](strategy.url, Auth.middleware.applyCSRF, (req, res, next) => __awaiter(this, void 0, void 0, function* () {
                    let opts = {
                    ...
                    ({ opts } = yield plugins.hooks.fire('filter:auth.options', { req, res, opts }));
                    passport.authenticate(strategy.name, opts).default(req, res, next);
                }));
            }
