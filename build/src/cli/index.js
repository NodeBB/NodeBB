...
try {
    ...
    fs.accessSync(path_1.default.join(paths.nodeModules, 'semver/package.json'), fs.constants.R_OK);
    const semver = require('semver');
    const defaultPackage = require('../../../install/package.json');
    const checkVersion = function (packageName) {
        const { version } = JSON.parse(fs.readFileSync(path_1.default.join(paths.nodeModules, packageName, 'package.json'), 'utf8'));
...
const configFile = path_1.default.resolve(paths.baseDir, nconf_1.default.get('config') || 'config.json');
...
program
    ...
    .action(() => {
    require('./running').start(Object.assign(Object.assign({}, program.opts()), { log: true }));
});
program
    .command('dev', null, {
    noHelp: true,
    ...
    require('./running').start(Object.assign(Object.assign({}, program.opts()), { dev: true }));
});
program
    .command('stop')
...
// user
program
    .addCommand(require('./user').default());
// upgrades
program
