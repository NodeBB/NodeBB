...
topicsController.get = function getTopic(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (req.params.post_index === 'unread') {
            postIndex = yield topics.getUserBookmark(tid, req.uid);
        }
        ...
        yield topics.getTopicWithPosts(topicData, set, req.uid, start, stop, reverse);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (allow) {
            ...
            if (!req.session.tids_viewed[tid] || req.session.tids_viewed[tid] < now - interval) {
                yield topics.increaseViewCount(tid);
...
function markAsRead(req, tid) {
    return __awaiter(this, void 0, void 0, function* () {
        if (req.loggedIn) {
            const markedRead = yield topics.markAsRead([tid], req.uid);
...
function buildBreadcrumbs(topicData) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const parentCrumbs = yield helpers.buildCategoryBreadcrumbs(topicData.category.parentCid);
...
function addOldCategory(topicData, userPrivileges) {
    return __awaiter(this, void 0, void 0, function* () {
        if (userPrivileges.isAdminOrMod && topicData.oldCid) {
            topicData.oldCategory = yield categories.getCategoryFields(topicData.oldCid, ['cid', 'name', 'icon', 'bgColor', 'color', 'slug']);
        }
    });
}
...
function addOGImageTags(res, topicData, postAtIndex) {
    return __awaiter(this, void 0, void 0, function* () {
        const uploads = postAtIndex ? yield posts.uploads.listWithSizes(postAtIndex.pid) : [];
        const images = uploads.map((upload) => {
            upload.name = `${url + upload_url}/${upload.name}`;
            return upload;
        });
        if (topicData.thumbs) {
            const thumbs = topicData.thumbs.filter((t) => t && images.every((img) => path_1.default.normalize(img.name) !== path_1.default.normalize(url + t.url)));
...
topicsController.teaser = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const canRead = yield privileges.topics.can('topics:read', tid, req.uid);
        if (!canRead) {
        ...
        const postData = yield posts.getPostSummaryByPids([pid], req.uid, { stripTags: false });
        if (!postData.length) {
            return res.status(404).json('not-found');
        }
        res.json(postData[0]);
