...
categoriesController.getAll = async function (req, res) {
	...
	async function getRootAndChildren() {
		...
		// @ts-ignore
		const childCids = _.flatten(await Promise.all(rootChildren.map((cid: string) => categories.getChildrenCids(cid))));
...
async function buildBreadcrumbs(categoryData, url: string) {
	if (!categoryData) {
		return;
	}
	const breadcrumbs = [
