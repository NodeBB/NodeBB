...
eventsController.get = async function (req, res) {
	...
	from = from && from.setHours(0, 0, 0, 0) as unknown as Date; // setHours returns a unix timestamp (Number, not Date)
	to = to && to.setHours(23, 59, 59, 999) as unknown as Date; // setHours returns a unix timestamp (Number, not Date)
