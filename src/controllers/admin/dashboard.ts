...
dashboardController.getAnalytics = async (req, res, next) => {
	...
	if (req.query.sets) {
		...
		sets = sets.filter((set: string) => validSets.includes(set));
	} else {
		sets = validSets;
	}

	const method = req.query.units === 'days' ? analytics.getDailyStatsForSet : analytics.getHourlyStatsForSet;
	let payload = await Promise.all(sets.map((set: string) => method(`analytics:${set}`, until, count)));
...
async function getStatsForSet(set: string, field: string) {
	const terms = {
		day: 86400000,
		week: 604800000,
		month: 2592000000,
...
async function getStatsFromAnalytics(set: string, field: string) {
	const today = new Date();
	today.setHours(0, 0, 0, 0);

	const data = await analytics.getDailyStatsForSet(`analytics:${set}`, today, 60);
	const sum = (arr: number[]) => arr.reduce((memo: number, cur: number) => memo + cur, 0);
	const results = {
...
function calculateDeltas(results) {
	...
	function increasePercent(last: number, now: number) {
...
async function getGlobalField(field: string) {
