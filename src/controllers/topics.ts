...
function generateQueryString(query: string) {
	const qString = qs.stringify(query);
	return qString.length ? `?${qString}` : '';
}

function calculatePageFromIndex(postIndex: number, settings) {
	return 1 + Math.floor((postIndex - 1) / settings.postsPerPage);
}

function calculateStartStop(page: number, postIndex: number, settings) {
	let startSkip = 0;
...
async function markAsRead(req, tid: string) {
	if (req.loggedIn) {
