...
function continueLogin(strategy: string, req, res, next: Function) {
	passport.authenticate(strategy, async (err, userData, info) => {
		...
		if (userData.passwordExpiry && userData.passwordExpiry < Date.now()) {
			...
			(res.locals.redirectAfterLogin || redirectAfterLogin).default(req, res, `${nconf.get('relative_path')}/reset/${code}`);
		} else {
...
function redirectAfterLogin(req, res, destination: string) {
	if (req.body.noscript === 'true') {
...
authenticationController.doLogin = async function (req, uid: string) {
	if (!uid) {
		return;
	}
...
authenticationController.onSuccessfulLogin = async function (req, uid: number) {
	/*
...
authenticationController.localLogin = async function (req, username: string, password: string, next: Function) {
	if (!username) {
...
const destroyAsync = util.promisify((req, callback: Function) => req.session.destroy(callback));
const logoutAsync = util.promisify((req, callback: Function) => req.logout(callback));
...
async function getBanError(uid: string) {
	try {
