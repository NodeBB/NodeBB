...
async function byType(type: string, req, res, next: Function) {
...
userController.getUserDataByField = async function (callerUid: string, field: string, fieldValue: string) {
	let uid: any = null;
	if (field === 'uid') {
		uid = fieldValue;
...
userController.getUserDataByUID = async function (callerUid: string, uid: string) {
	if (!parseInt(uid, 10)) {
