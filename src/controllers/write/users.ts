...
const hasAdminPrivilege = async (uid: string, privilege: string) => {
...
const getSessionAsync = util.promisify((sid: string, callback: Function) => {
	db.sessionStore.get(sid, (err: Error, sessionObj) => callback(err, sessionObj || null));
});
...
const prepareExport = async (req, res) => {
	const [extension] = exportMetadata.get(req.params.type) as any[];
...
Users.getExportByType = async (req, res) => {
	const [extension, mime] = exportMetadata.get(req.params.type) as any[];
	const filename = `${req.params.uid}_${req.params.type}.${extension}`;
