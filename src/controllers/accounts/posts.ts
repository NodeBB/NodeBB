...
import categories from '../../categories';const privileges = require('../../privileges');
...
const templateToData = {
	'account/bookmarks': {
		...
		getSets: function (callerUid: string, userData) {
	...
	'account/posts': {
		...
		crumb: '[[global:posts]]',
		getSets: async function (callerUid: string, userData) {
			// @ts-ignore
	...
	'account/best': {
		...
		getSets: async function (callerUid: string, userData) {
			// @ts-ignore
			const cids = await categories.getCidsByPrivilege('categories:cid', callerUid, 'topics:read');
			return cids.map((c:string) => `cid:${c}:uid:${userData.uid}:pids:votes`);
		},
		getTopics: async (sets, req, start: number, stop: number) => {
		...
		getItemCount: async (sets: Array<any>) => {
	...
	'account/controversial': {
		...
		getTopics: async (sets: Array<any>, req, start: number, stop: number) => {
	...
	'account/watched': {
		...
		getTopics: async function (set, req, start: number, stop: number) {
			...
			tids = tids.map((tid: string, i: number) => ({ tid: tid, score: scores[i] }))
...
async function getPostsFromUserSet(template: string, req, res, next) {
...
async function getItemData(sets, data, req, start: number, stop: number) {
	if (data.getTopics) {
