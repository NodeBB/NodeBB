...
helpers.getUserDataByUserSlug = async function (userslug: string, callerUID: string, query = {}) {
...
async function getAllData(uid: string, callerUID: string) {
...
async function getCounts(userData, callerUID: string) {
	const { uid } = userData;
	// @ts-ignore
	const cids = await categories.getCidsByPrivilege('categories:cid', callerUID, 'topics:read');
	const promises = {
		posts: db.sortedSetsCardSum(cids.map((c: string) => `cid:${c}:uid:${uid}:pids`)),
		best: Promise.all(cids.map(async (c: string) => db.sortedSetCount(`cid:${c}:uid:${uid}:pids:votes`, 1, '+inf'))),
		controversial: Promise.all(cids.map(async (c: string) => db.sortedSetCount(`cid:${c}:uid:${uid}:pids:votes`, '-inf', -1))),
		topics: db.sortedSetsCardSum(cids.map((c: string) => `cid:${c}:uid:${uid}:tids`)),
	} as any;
	...
	counts.best = counts.best.reduce((sum: number, count: number) => sum + count, 0);
	counts.controversial = counts.controversial.reduce((sum: number, count: number) => sum + count, 0);
...
async function getProfileMenu(uid: string, callerUID: string) {
	const links = [{
		id: 'info',
		route: 'info',
...
function filterLinks(links: Array<any>, states) {
