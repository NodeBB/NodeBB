...
helpers.getUserDataByUserSlug = function (userslug, callerUID, callback) {
	async.waterfall([
		...
		function (results, next) {
			...
			userData['cover:url'] = userData['cover:url'] ? (userData['cover:url].startsWith('http') ? userData['cover:url'] : (nconf.get('relative_path') + userData['cover:url']) ) : require('../../coverPhoto').getDefaultProfileCover(userData.uid);
