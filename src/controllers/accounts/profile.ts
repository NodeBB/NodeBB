...
async function getLatestPosts(callerUid: string, userData) {
	return await getPosts(callerUid, userData, 'pids');
}

async function getBestPosts(callerUid: string, userData) {
	return await getPosts(callerUid, userData, 'pids:votes');
}

async function getPosts(callerUid: string, userData, setSuffix) {
	// @ts-ignore
