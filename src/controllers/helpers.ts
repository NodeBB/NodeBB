...
helpers.noScriptErrors = async function (req, res, error, httpStatus: number) {
...
helpers.buildQueryString = function (query, key: string, value) {
...
helpers.buildFilters = function (url: string, filter: string, query) {
	return [{
...
helpers.buildTerms = function (url: string, term: string, query: string) {
	return [{
		name: '[[recent:alltime]]',
...
helpers.notAllowed = async function (req, res, error: Error) {
...
helpers.redirect = function (res, url, permanent: string) {
...
function prependRelativePath(url: string) {
	return url.startsWith('http://') || url.startsWith('https://') ?
		url : relative_path + url;
}

helpers.buildCategoryBreadcrumbs = async function (cid: string) {
...
helpers.buildBreadcrumbs = function (crumbs: Array<any>) {
	const breadcrumbs = [
		{
			text: '[[global:home]]',
			url: `${relative_path}/`,
...
helpers.getCategories = async function (set: string, uid: string, privilege: string, selectedCid: string) {
...
helpers.getCategoriesByStates = async function (uid: string, selectedCid: string, states: string[], privilege = 'topics:read') {
	const cids = await categories.getAllCidsFromSet('categories:cid');
	return await getCategoryData(cids, uid, selectedCid, states, privilege);
};

async function getCategoryData(cids: string[], uid: string, selectedCid: string, states: string[], privilege: string) {
...
helpers.getVisibleCategories = async function (params) {
	...
	const cidToAllowed = _.zipObject(cids, allowed.map((allowed, i: number) => isAdmin || isModerator[i] || allowed));
...
helpers.getSelectedCategory = async function (cids: Array<any>) {
...
function checkVisibleChildren(c, cidToAllowed: string, cidToWatchState: string, states: string[]) {
...
helpers.getHomePageRoutes = async function (uid: string) {
	const routes = [
		{
			route: 'categories',
			name: 'Categories',
...
helpers.formatApiResponse = async (statusCode: number, res, payload) => {
	...
	if (String(statusCode).startsWith('2')) {
		...
		if ((global as any).env === 'development') {
			...
			(process as any).stdout.write(payload.stack as string);
		}
...
helpers.generateError = async (statusCode, message: string, res) => {
