...
categoryController.get = async function (req, res, next) {
	...
	let topicIndex: number | string = utils.isNumber(req.params.topic_index) ? parseInt(req.params.topic_index, 10) - 1 : 0;
	...
	if (!userSettings.usePagination) {
		...
		const index = Math.max(parseInt((topicIndex || 0) as unknown as string, 10), 0);
