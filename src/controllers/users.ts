...
usersController.getUsers = async function (set, uid: string, query) {
	const setToData = {
...
usersController.getUsersAndCount = async function (set: string, uid: string, start: number, stop: number | string) {
	...
	async function getUsers() {
		if (set === 'users:online') {
			...
			const [userStatus, userData] = await Promise.all([
				db.getObjectsFields(uids.map((uid: string) => `user:${uid}`), ['status']),
				user.getUsers(uids, uid),
			]);

			userData.forEach((user, i: string) => {
				if (user) {
