...
export default  function (User) {
	...
	async function isReady(uid, cid, field) {
		...
		if (userData.mutedUntil > now) {
			let muteLeft = ((userData.mutedUntil - now) / (1000 * 60)) as number | string;
			if (muteLeft as number > 60) {
				muteLeft = (muteLeft as number / 60).toFixed(0);
				throw new Error(`[[error:user-muted-for-hours, ${muteLeft}]]`);
			} else {
				throw new Error(`[[error:user-muted-for-minutes, ${(muteLeft as number).toFixed(0)}]]`);
			}
		}
