...
User.isModeratorOfAnyCategory = async function (uid: string) {
	const cids = await User.getModeratedCids(uid);
	return Array.isArray(cids) ? !!cids.length : false;
};

User.isAdministrator = async function (uid: string) {
	return await privileges.users.isAdministrator(uid);
};

User.isGlobalModerator = async function (uid: string) {
	return await privileges.users.isGlobalModerator(uid);
};

User.getPrivileges = async function (uid: string) {
...
User.isPrivileged = async function (uid: string) {
...
User.isAdminOrGlobalMod = async function (uid: string) {
...
User.getModeratedCids = async function (uid: string) {
	if (parseInt(uid, 10) <= 0) {
		return [];
	}
	const cids = await categories.getAllCidsFromSet('categories:cid');
