...
(process as any).on('message', async (msg: any) => {
	if (msg && msg.uid) {
		...
		let chatData : any[] = [];
		await batch.processSortedSet(`uid:${targetUid}:chat:rooms`, async (roomIds: string[]) => {
...
async function getRoomMessages(uid: string, roomId: string) {
	const batch = require('../../batch');
	let data : any[] = [];
	await batch.processSortedSet(`uid:${uid}:chat:room:${roomId}:mids`, async (mids: string[]) => {
...
async function getSetData(set: string, keyPrefix: string, uid: string) {
	const privileges = require('../../privileges');
	const batch = require('../../batch');
	let data : any[] = [];
	await batch.processSortedSet(set, async (ids: string[]) => {
		if (keyPrefix === 'post:') {
		...
		if (keyPrefix === 'post:') {
			objData = objData.map((o: string) => _.pick(o, ['pid', 'content', 'timestamp']));
		} else if (keyPrefix === 'topic:') {
			objData = objData.map((o: string) => _.pick(o, ['tid', 'title', 'timestamp']));
		} else if (keyPrefix === 'user:') {
			objData = objData.map((o: string) => _.pick(o, ['uid', 'username']));
		}
