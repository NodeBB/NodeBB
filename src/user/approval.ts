...
export default  function (User) {
	...
	async function sendNotificationToAdmins(username: string) {
	...
	async function markNotificationRead(username: string) {
		const nid = `new_register:${username}`;
		const uids = await groups.getMembers('administrators', 0, -1);
		const promises = uids.map((uid: string) => notifications.markRead(nid, uid));
		await Promise.all(promises);
	}

	User.rejectRegistration = async function (username: string) {
		await removeFromQueue(username);
		await markNotificationRead(username);
	};
