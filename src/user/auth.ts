...
export default function (User) {
	...
	const getSessionFromStore = util.promisify(
		(sid: string, callback: Function) => db.sessionStore.get(sid, (err: Error, sessObj) => callback(err, sessObj || null))
	);
	const sessionStoreDestroy = util.promisify(
		(sid: string, callback: Function) => db.sessionStore.destroy(sid, (err: Error) => callback(err))
	);
	...
	User.auth.getSessions = async function (uid: string, curSessionId: string) {
		...
		let sessions = await Promise.all(sids.map((sid: string) => getSessionFromStore(sid)));
	...
	async function cleanExpiredSessions(uid: string) {
		...
		const expiredUUIDs : any[] = [];
		const expiredSids : any[] = [];
	...
	async function revokeSessionsAboveThreshold(uid: string, maxUserSessions: number) {
		...
		if (activeSessions.length > maxUserSessions) {
			...
			await Promise.all(sessionsToRevoke.map((sessionId: string) => User.auth.revokeSession(sessionId, uid)));
		}
	}
	...
	User.auth.revokeAllSessions = async function (uids: string[], except: string) {
		uids = Array.isArray(uids) ? uids : [uids];
		const sids = await db.getSortedSetsMembers(uids.map(uid => `uid:${uid}:sessions`));
		const promises : any[] = [];
		uids.forEach((uid, index) => {
			const ids = sids[index].filter((id: string) => id !== except);
			if (ids.length) {
				promises.push(ids.map((s: string) => User.auth.revokeSession(s, uid)));
			}
		});
	...
	User.auth.deleteAllSessions = async function () {
		await batch.processSortedSet('users:joindate', async (uids: string[]) => {
			const sessionKeys = uids.map((uid: string) => `uid:${uid}:sessions`);
			const sessionUUIDKeys = uids.map((uid: string) => `uid:${uid}:sessionUUID:sessionId`);
			const sids = _.flatten(await db.getSortedSetRange(sessionKeys, 0, -1));

			await Promise.all([
				db.deleteAll(sessionKeys.concat(sessionUUIDKeys)),
				...sids.map((sid: string) => sessionStoreDestroy(sid)),
			]);
		}, { batch: 1000 });
	};
};
