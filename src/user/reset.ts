...
UserReset.generate = async function (uid: string) {
...
async function canGenerate(uid: string) {
...
UserReset.cleanByUid = async function (uid: string | number) {
	const tokensToClean : any[] = [];
	...
	await batch.processSortedSet('reset:issueDate', async (tokens) => {
		...
		for (const [code, result] of Object.entries(results)) {
			if (parseInt(result as string, 10) === uid) {
