...
module.exports = function (User) {
	...
	function validatePerPage(limit, set) {
		limit = limit || 20;
		set = parseInt(set, 10);
		if (set <= 1 || set > limit) {
			throw new Error(`[[error:invalid-pagination-value, 2, ${limit}]]`);
		}
		return set;
	}

	User.saveSettings = async function (uid, data) {
		const postsPerPage = validatePerPage(meta.config.maxPostsPerPage, data.postsPerPage);
		const topicsPerPage = validatePerPage(meta.config.maxTopicsPerPage, data.topicsPerPage);
		...
		const result = await plugins.hooks.fire('filter:user.saveSettings', { settings, data });
