...
UserNotifications.get = async function (uid: string) {
...
UserNotifications.getDailyUnread = async function (uid: string) {
	return await UserNotifications.getUnreadInterval(uid, 'day');
};

UserNotifications.getUnreadCount = async function (uid: string) {
	if (parseInt(uid, 10) <= 0) {
		return 0;
	}
...
UserNotifications.deleteAll = async function (uid: string) {
	if (parseInt(uid, 10) <= 0) {
		return;
	}
	await db.deleteAll([
...
UserNotifications.pushCount = async function (uid: string) {
