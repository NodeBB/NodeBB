...
Settings.get = async function (hash) {
	...
	data = { ...data, ...await Settings.sortedLists.get(hash) };
...
Settings.set = async function (hash, values, quiet) {
	...
	if (sortedLists.length) {
		const sortedListValues = sortedLists.reduce((memo, cur) => {
			memo[cur] = values[cur];
			delete values[cur];
			return memo;
		}, {});
		Settings.sortedLists.set(hash, sortedLists, sortedListValues);
	}
...
Settings.sortedLists = {};

Settings.sortedLists.get = async (hash) => {
	const data = {};
...
Settings.sortedLists.set = async (hash, sortedLists, values) => {
