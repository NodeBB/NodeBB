<div class="">
	...
	<div class="d-flex gap-2 mb-3 align-items-center gap-2">
		<label class="form-label text-nowrap mb-0">{{tx("modules:chat.default-notification-setting")}}</label>
		<select component="chat/room/notification/setting" class="form-select" style="width: 200px;">
			<option value="1" {{{ if (room.notificationSetting == "1") }}}selected{{{ end }}}>{{tx("modules:chat.notification-setting-none")}}</option>
			<option value="2" {{{ if (room.notificationSetting == "2") }}}selected{{{ end }}}>{{tx("modules:chat.notification-setting-at-mention-only")}}</option>
			<option value="3" {{{ if (room.notificationSetting == "3") }}}selected{{{ end }}}>{{tx("modules:chat.notification-setting-all-messages")}}</option>
		</select>
	</div>
	<div class="mb-3 d-flex gap-2 align-items-center">
		<label class="form-label">{{tx("modules:chat.join-leave-messages")}}</label>
	...
	<input component="chat/manage/user/add/search" class="form-control" type="text" placeholder="{{tx("global:user-search-prompt")}}" />
	<p class="text-danger"></p>
	<p class="form-text">{{tx("modules:chat.add-user-help")}}</p>
	...
	<div class="row">
		<div class="col-12 {{{ if (user.isAdmin && room.public) }}}col-md-6{{{ end }}}">
			...
			<ul component="chat/manage/user/list" class="list-group overflow-auto" style="max-height: 300px;">
				<li class="list-group-item"><i class="fa fa-spinner fa-spin"></i> {{tx("modules:chat.retrieving-users")}}</li>
			</ul>
		</div>
		{{{ if (user.isAdmin && room.public) }}}
		<div class="col-12 col-md-6 d-flex flex-column">
			<label class="form-label">{{tx("modules:chat.select-groups")}}</label>
