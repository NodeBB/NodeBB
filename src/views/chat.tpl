	...
	<div class="modal-dialog m-0">
		<div class="modal-content m-4" component="chat/message/window">
			<div class="modal-header d-flex gap-4 justify-content-between">
				<div id="chat-room-title-{roomId}" class="fs-6 flex-grow-1 fw-semibold tracking-tight text-truncate text-nowrap" component="chat/room/name" data-icon="{icon}">{{{ if ./roomName }}}<i class="fa {icon} text-muted"></i> {roomName}{{{ else }}}{{tx(./chatWithMessage)}}{{{ end}}}</div>
				<div class="d-flex gap-1 align-items-center">
					<button type="button" class="btn btn-ghost btn-sm d-none d-md-flex align-self-stretch align-items-center" data-action="maximize" title="{{tx("modules:chat.maximize")}}" data-bs-toggle="tooltip" data-bs-placement="bottom">
