<div class="acp-page-container px-lg-4">
	...
	<div component="settings/main/header" class="row border-bottom py-2 m-0 sticky-top acp-page-main-header align-items-center">
		<div class="col-12 col-md-8 px-0 mb-1 mb-md-0 d-flex justify-content-between align-items-center">
			<h4 class="fw-bold tracking-tight mb-0">{{tx("admin/manage/groups:edit-group")}}</h4>
		...
		<div class="col-12 col-md-4 px-0 px-md-3 ">
			<button id="save" class="btn btn-primary btn-sm btn btn-primary btn-sm fw-semibold ff-secondary w-100 text-center text-nowrap">{{tx("admin/admin:save-changes")}}</button>
		</div>
	</div>
	...
	<div class="row m-0" data-groupname="{group.displayName}">
		<div class="col-12 col-md-8 px-0">
			<div class="group-settings-form">
				<div class="mb-3">
					<label class="form-label" for="change-group-name">{{tx("admin/manage/groups:edit.name")}}</label>
				...
				<div class="mb-3">
					<label class="form-label" for="change-group-desc">{{tx("admin/manage/groups:edit.description")}}</label>
				...
				<div class="mb-3 d-flex flex-column gap-1">
					<div class="d-flex gap-2">
						<label class="form-label" for="category-image">
							{{tx("admin/manage/groups:icon-and-title")}}
						</label>
					...
					<div class="d-flex gap-2 align-items-center">
						<div class="pointer border rounded-1 p-1 d-flex align-items-center justify-content-center" id="group-icon-container" style="width:40px; height:40px;" data-bs-toggle="tooltip" title="{{tx("admin/admin:select-icon")}}">
				...
				<div class="mb-3 d-flex justify-content-between align-items-center gap-2">
					<label class="form-label text-nowrap" for="change-group-label-color">{{tx("admin/manage/groups:edit.label-color")}}</label>
				...
				<div class="mb-3 d-flex justify-content-between align-items-center gap-2">
					<label class="form-label text-nowrap" for="change-group-text-color">{{tx("admin/manage/groups:edit.text-color")}}</label>
					<!-- added this to match the height of other blocks -->
					<input type="text" class="form-control invisible" style="width: 0px;">
				...
				<div class="mb-3">
					<div class="form-check form-switch">
						...
						<label class="form-check-label" for="group-userTitleEnabled">{{tx("admin/manage/groups:edit.show-badge")}}</label>
					</div>
				</div>
				...
				<div class="mb-3">
					<div class="form-check form-switch">
						...
						<label class="form-check-label" for="group-private">{{tx("groups:details.private")}}</label>
					</div>
					<p class="form-text">
						{{tx("admin/manage/groups:edit.private-details")}}
					</p>
					{{{ if !allowPrivateGroups }}}
					<p class="form-text">
						{{tx("admin/manage/groups:edit.private-override")}}
					</p>
					{{{ end }}}
				</div>
				...
				<div class="mb-3">
					<div class="form-check form-switch">
						...
						<label class="form-check-label" for="group-disableJoinRequests">{{tx("admin/manage/groups:edit.disable-join")}}</label>
					</div>
				</div>
				...
				<div class="mb-3">
					<div class="form-check form-switch">
						...
						<label class="form-check-label" for="group-disableLeave">{{tx("admin/manage/groups:edit.disable-leave")}}</label>
					</div>
				</div>
				...
				<div class="mb-3">
					<div class="form-check form-switch">
						...
						<p class="form-text">
							{{tx("admin/manage/groups:edit.hidden-details")}}
						</p>
					</div>
				</div>
				...
				<div class="d-flex flex-column gap-1 mb-3">
					<label class="form-label" for="memberPostCids" for="memberPostCids">{{tx("groups:details.member-post-cids")}}</label>
				...
				<div class="d-flex flex-column gap-2">
					<label class="form-label">{{tx("admin/manage/groups:edit.members")}}</label>
		...
		<div class="col-12 col-md-4 px-0 px-md-3 acp-sidebar">
			<div class="p-2 d-flex flex-column text-bg-light border rounded-1 gap-1">
				<a href="{config.relative_path}/admin/manage/groups" class="btn btn-ghost btn-sm d-flex gap-2 align-items-center"><i class="fa fa-fw fa-chevron-left text-primary"></i> {{tx("admin/manage/groups:back-to-groups")}}</a>
				...
				<div class="edit-privileges-selector w-100">
					<div component="category-selector" class="dropdown w-100">
						<button type="button" class="btn btn-ghost btn-sm d-flex gap-2 align-items-center flex-fill dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							<i class="fa fa-fw fa-lock text-primary"></i> <span>{{tx("admin/manage/groups:privileges")}}</span> <span class="caret"></span>
						</button>
						...
						<div class="dropdown-menu p-1">
							...
							<ul component="category/list" class="list-unstyled mb-0 text-sm category-dropdown-menu dropdown-menu-end ghost-scrollbar" role="menu">
								<li component="category/no-matches" role="presentation" class="category hidden">
									<a class="dropdown-item" role="menuitem">{{tx("search:no-matches")}}</a>
								</li>
								{{{ each categories }}}
				...
				<a href="{config.relative_path}/api/admin/groups/{group.nameEncoded}/csv" class="btn btn-ghost btn-sm d-flex gap-2 align-items-center">
					<i class="fa fa-fw fa-file-csv text-primary"></i> {{tx("admin/manage/groups:members-csv")}}</a>

				<a href="{config.relative_path}/groups/{group.slug}" class="btn btn-ghost btn-sm d-flex gap-2 align-items-center">
					<i class="fa fa-fw fa-eye text-primary"></i> {{tx("admin/manage/groups:view-group")}}
				</a>

				<hr class="my-1"/>

				<button data-action="delete" class="btn btn-ghost btn-sm d-flex gap-2 align-items-center">
					<i class="fa fa-fw fa-trash text-danger"></i> {{tx("admin/manage/groups:delete")}}
				</button>
			</div>
		</div>
	</form>
