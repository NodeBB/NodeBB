...
<div class="tags d-flex flex-column gap-2 px-lg-4">
	<div class="d-flex border-bottom py-2 m-0 sticky-top acp-page-main-header align-items-center justify-content-between flex-wrap gap-2">
		<div class="">
			<h4 class="fw-bold tracking-tight mb-0">{{tx("admin/extend/rewards:rewards")}}</h4>
		</div>
		<div class="d-flex align-items-center gap-1">
			<button id="new" class="btn btn-light btn-sm text-nowrap" type="button">
				<i class="fa fa-fw fa-plus"></i> {{tx("admin/extend/rewards:add-reward")}}
			</button>
						...
						<div class="d-flex gap-1 mb-3 flex-wrap">
							<form class="main d-flex gap-1 flex-wrap">
								<div class="card card-body m-0 if-block border-info border border-2">
									<label class="form-label" for="condition-if-users">{{tx("admin/extend/rewards:condition-if-users")}}</label>
								...
								<div class="card card-body m-0 this-block border-warning border border-2">
									<label class="form-label" for="condition-is">{{tx("admin/extend/rewards:condition-is")}}</label>
								...
								<div class="card card-body m-0 then-block border-primary border border-2">
									<label class="form-label" for="condition-then">{{tx("admin/extend/rewards:condition-then")}}</label>
							...
							<form class="rewards flex-1">
								<div class="inputs card card-body m-0 h-100 reward-block border-success border border-2"><div class="d-flex h-100 align-items-center">{{tx("admin/extend/rewards:select-reward")}}</div></div>
							</form>
						</div>
						...
						<div class="d-flex justify-content-between align-items-center gap-2">
							<form class="main d-flex gap-1 align-items-start gap-2">
								<div class="d-flex flex-column gap-0">
									<label class="form-label" for="claimable">{{tx("admin/extend/rewards:max-claims")}}</label>
									<p class="form-text mb-0">
										{{tx("admin/extend/rewards:zero-infinite")}}
									</p>
								</div>
							...
							<div class="d-flex gap-1 align-self-start">
								<button class="btn btn-light btn-sm toggle text-nowrap disable {{{ if active.disabled }}}hidden{{{ end }}}"><i class="fa fa-ban text-danger"></i> {{tx("admin/extend/rewards:disable")}}</button>

								<button class="btn btn-light btn-sm toggle text-nowrap enable {{{ if !active.disabled }}}hidden{{{ end }}}"><i class="fa fa-check text-success"></i> {{tx("admin/extend/rewards:enable")}}</button>

								<button class="btn btn-light btn-sm text-nowrap delete"><i class="fa fa-trash text-danger"></i> {{tx("admin/extend/rewards:delete")}}</button>
							</div>
						</div>
					</div>
				</div>
