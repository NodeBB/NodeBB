<div class="d-flex justify-content-between mb-2">
	{{{ if group.isOwner }}}
	<button component="groups/members/add" type="button" class="btn btn-primary btn-sm text-nowrap" title="{{tx("groups:details.add-member")}}">{{tx("groups:details.add-member")}}</button>
	{{{ end }}}
	<div>
		...
		<tr data-uid="{group.members.uid}">
			<td class="member-name d-flex justify-content-between align-items-center">
				<div class="d-flex align-items-center gap-2">
					...
					<a href="{config.relative_path}/user/{group.members.userslug}">{group.members.username}</a> <i title="{{tx("groups:owner")}}" class="user-owner-icon fa fa-star text-warning {{{ if !group.members.isOwner }}}invisible{{{ end }}}"></i>
				</div>
