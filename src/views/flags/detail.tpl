		...
		<div class="d-grid gap-1">
			...
			<div class="dropdown dropend" data-uid="{target.uid}">
				...
				<ul class="dropdown-menu p-1 text-sm" role="menu">
					<li><a class="dropdown-item rounded-1" href="{config.relative_path}/uid/{target.uid}" role="menuitem">{{tx("flags:view-profile")}}</a></li>
					{{{ if !config.disableChat }}}
						<li><a class="dropdown-item rounded-1" href="#" data-action="chat" role="menuitem">{{tx("flags:start-new-chat")}}</a></li>
					{{{ end }}}
			...
			<div class="d-flex flex-column gap-1">
				...
				<div>
					<ul class="list-unstyled">
						{{{ each ./fields }}}
						<li>
							{{tx(concat("flags:", @key))}}{{{ if @value }}} &rarr; <span class="fw-semibold">{{tx(@value)}}</span>{{{ end }}}
						</li>
						{{{ end }}}
						{{{ each ./meta }}}
						<li>
							{{tx(./key)}}{{{ if ./value }}} &rarr; <span class="fw-semibold">{./value}</span>{{{ end }}}
						</li>
						{{{ end }}}
					</ul>
				</div>
		...
		<div class="d-flex flex-column gap-4">
			...
			<div class="pb-4 border-bottom">
				<div class="d-flex align-items-center">
					...
					<button class="btn btn-ghost ff-secondary border" data-action="addEditNote">{{tx("flags:add-note")}}</button>
				</div>
