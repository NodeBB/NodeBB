...
pubsub.on('dbal:hash:cache:del', function (keys) {
	keys.forEach(key => cache.del(key));
});

pubsub.on('dbal:hash:cache:reset', function () {
	cache.reset();
});
...
cache.delObjectCache = function (keys) {
	...
	pubsub.publish('dbal:hash:cache:del', keys);
	keys.forEach(key => cache.del(key));
};

cache.resetObjectCache = function () {
	pubsub.publish('dbal:hash:cache:reset');
	cache.reset();
};
