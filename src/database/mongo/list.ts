...
export default  function (module) {
	...
	module.listPrepend = async function (key: string, value: string | string[]) {
		if (!key) {
			return;
		}
	...
	module.listAppend = async function (key: string, value: string | string[]) {
		if (!key) {
			return;
		}
		value = Array.isArray(value) ? value : [value];
	...
	async function listPush(key: string, values: string[], position?) {
	...
	module.listRemoveAll = async function (key: string, value: string[] | string) {
		if (!key) {
			return;
		}
	...
	module.getListRange = async function (key: string, start: number, stop: number) {
		if (!key) {
			return;
		}
