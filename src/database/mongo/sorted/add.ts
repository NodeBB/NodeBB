...
export default function (module) {
	...
	module.sortedSetAdd = async function (key, score: Array<number> | number | string, value: Array<number> | number) {
		...
		try {
			await module.client.collection('objects').updateOne({ _key: key, value: value }, { $set: { score: parseFloat(score as string) } }, { upsert: true });
		} catch (err: any) {
	...
	module.sortedSetsAdd = async function (keys: string[], scores: string[], value: string) {
