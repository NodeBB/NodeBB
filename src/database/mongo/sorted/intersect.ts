...
export default  function (module) {
	module.sortedSetIntersectCard = async function (keys: string[]) {
	...
	async function countSets(sets: string[], limit?: number) {
		const objects = module.client.collection('objects');
	...
	async function getSortedSetRevIntersect(params) {
		...
		const simple = params.weights.filter((w: number) => w === 1).length === 1 && params.limit !== 0;
	...
	async function intersectAggregate(params) {
		...
		params.weights.forEach((weight: number, index: number) => {
			if (weight !== 1) {
				pipeline.push({
					$project: {
						value: 1,
