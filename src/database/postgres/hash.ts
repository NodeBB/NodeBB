...
export default  function (module) {
	...
	module.setObject = async function (key: string, data) {
		if (!key || !data) {
			return;
		}
	...
	module.setObjectBulk = async function (...args) {
		...
		if (Array.isArray(args[1])) {
			...
			data = args[0].map((key: string, i: number) => [key, args[1][i]]);
		}
		await module.transaction(async (client) => {
	...
	module.setObjectField = async function (key: string, field: string, value) {
		if (!field) {
			return;
		}
	...
	module.getObject = async function (key: string[], fields = []) {
		if (!key) {
			return null;
		}
		if (fields.length) {
...
SELECT h."data"
 ...
 LIMIT 1`,
	...
	module.getObjects = async function (keys: string[], fields = []) {
...
SELECT h."data"
 ...
 ORDER BY k.i ASC`,
	...
	module.getObjectField = async function (key: string, field: string) {
		if (!key) {
			return null;
		}
...
SELECT h."data"->>$2::TEXT f
 ...
 LIMIT 1`,
	...
	module.getObjectFields = async function (key: string, fields) {
		if (!key) {
			return null;
		}
...
SELECT (SELECT jsonb_object_agg(f, d."value")
 ...
 WHERE o."_key" = $1::TEXT`,
	...
	module.getObjectsFields = async function (keys: string[], fields) {
		if (!Array.isArray(keys) || !keys.length) {
			return [];
		}
...
SELECT (SELECT jsonb_object_agg(f, d."value")
 ...
 ORDER BY k.i ASC`,
	...
	module.getObjectKeys = async function (key: string) {
		if (!key) {
			return;
		}
...
SELECT ARRAY(SELECT jsonb_object_keys(h."data")) k
 ...
 LIMIT 1`,
	...
	module.getObjectValues = async function (key: string) {
		const data = await module.getObject(key);
		return data ? Object.values(data) : [];
	};

	module.isObjectField = async function (key: string, field: string) {
		if (!key) {
			return;
		}
...
SELECT (h."data" ? $2::TEXT AND h."data"->>$2::TEXT IS NOT NULL) b
 ...
 LIMIT 1`,
	...
	module.isObjectFields = async function (key: string, fields: string[]) {
		if (!key) {
			return;
		}
	...
	module.deleteObjectField = async function (key: string, field: string) {
		await module.deleteObjectFields(key, [field]);
	};

	module.deleteObjectFields = async function (key: string, fields: string[]) {
	...
	module.incrObjectField = async function (key: string, field: string) {
		return await module.incrObjectFieldBy(key, field, 1);
	};

	module.decrObjectField = async function (key: string, field: string) {
		return await module.incrObjectFieldBy(key, field, -1);
	};

	module.incrObjectFieldBy = async function (key: string, field: string, value) {
