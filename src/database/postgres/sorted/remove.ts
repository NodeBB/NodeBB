...
export default  function (module) {
	...
	module.sortedSetRemove = async function (key: string | string[], value) {
		if (!key) {
			return;
		}
...
DELETE FROM "legacy_zset"
 WHERE "_key" = ANY($1::TEXT[])
   AND "value" = $2::TEXT`,
	...
	module.sortedSetsRemoveRangeByScore = async function (keys: string[], min: number | string | null, max: number | string | null) {
		if (!Array.isArray(keys) || !keys.length) {
			return;
		}
