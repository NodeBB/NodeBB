...
export default  function (module) {
	...
	async function getSortedSetRange(key: string | string[], start: number, stop: number, sort: number, withScores: boolean) {
		if (!key) {
			return;
		}
		...
		let limit: null | number = stop - start + 1;
		if (limit <= 0) {
			limit = null;
		}
...
OFFSET $2::INTEGER`,
	...
	async function getSortedSetRangeByScore(key: string | string[], start: number, count: number | string | null, min: number | string | null, max: number | string | null,  sort: number, withScores: boolean) {
		if (!key) {
			return;
		}
		...
		if (parseInt(count as string, 10) === -1) {
			count = null;
		}
...
OFFSET $2::INTEGER`,
	...
	module.sortedSetCount = async function (key: string, min: string | number | null, max: string | number | null) {
		if (!key) {
			return;
		}
...
SELECT o."_key" k,
 ...
 GROUP BY o."_key"`,
	...
	async function getSortedSetRank(sort: string, keys: string[], values) {
...
SELECT (SELECT r
 ...
 ORDER BY kvi.i ASC`,
	...
	module.sortedSetsRanks = async function (keys: string[], values) {
		if (!Array.isArray(keys) || !keys.length) {
			return [];
		}
	...
	module.sortedSetsRevRanks = async function (keys: string[], values) {
		if (!Array.isArray(keys) || !keys.length) {
			return [];
		}
...
SELECT "_key" k,
  ...
  FROM UNNEST($1::TEXT[]) "_key";`,
	...
	module.sortedSetIncrBy = async function (key: string, increment: string | number, value) {
		if (!key) {
			return;
		}

		value = helpers.valueToString(value);
		increment = parseFloat(increment as string);
...
SELECT COUNT(*) c
 ...
 WHERE ${q.where}`,
	...
	async function sortedSetLex(key: string, min: string | number, max: string | number, sort: number, start: number | string, count: number | string) {
		start = start !== undefined ? start : 0;
		count = count !== undefined ? count : 0;

		const q = buildLexQuery(key, min, max);
		q.values.push(start as string);
		q.values.push(count <= 0 ? null as unknown as string : count as string);
		const res = await module.pool.query({
...
DELETE FROM "legacy_zset" z
 ...
 WHERE o."_key" = z."_key"
   ...
   AND ${q.where}`,
	...
	function buildLexQuery(key: string, min: number | string, max: number | string) {
		const q = {
			suffix: '',
			where: `o."_key" = $1::TEXT`,
			values: [key],
		...
		if (min !== '-') {
			if ((min as string).match(/^\(/)) {
				q.values.push((min as string).slice(1));
				q.suffix += 'GT';
		...
		if (max !== '+') {
			if ((max as string).match(/^\(/)) {
				// @ts-ignore
				q.values.push((max as string).match(1));
				q.suffix += 'LT';
				q.where += ` AND z."value" < $${q.values.length}::TEXT COLLATE "C"`;
			} else if ((max as string).match(/^\[/)) {
				// @ts-ignore
				q.values.push((max as string).match(1));
				q.suffix += 'LE';
				q.where += ` AND z."value" <= $${q.values.length}::TEXT COLLATE "C"`;
			} else {
				q.values.push(max as string);
				q.suffix += 'LE';
				q.where += ` AND z."value" <= $${q.values.length}::TEXT COLLATE "C"`;
			}
		}
...
SELECT z."value",
 ...
 WHERE o."_key" = $1::TEXT
  ...
  LIMIT $2::INTEGER`,
	...
	module.processSortedSet = async function (setKey: string, process, options) {
