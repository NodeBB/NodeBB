...
export default  function (Posts) {
	...
	async function deleteFromTopicUserNotification(postData) {
		...
		for (const [cid, posts] of Object.entries(postsByCategory) as any) {
			incrObjectBulk.push([`category:${cid}`, { post_count: -posts.length }] as any);
		}

		const postsByTopic = _.groupBy(postData, p => parseInt(p.tid, 10));
		const topicPostCountTasks : any[] = [];
		const topicTasks : any[] = [];
		const zsetIncrBulk : any[] = [];
		for (const [tid, posts] of Object.entries(postsByTopic) as any) {
			incrObjectBulk.push([`topic:${tid}`, { postcount: -posts.length }] as any);
