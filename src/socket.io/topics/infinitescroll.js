...
module.exports = function (SocketTopics) {
	SocketTopics.loadMore = function (socket, data, callback) {
		...
		async.waterfall([
			...
			function (results, next) {
				...
				var infScrollPostsPerPage = Math.max(0, Math.min(meta.config.postsPerPage, parseInt(data.postsPerPage, 10) || meta.config.postsPerPage));
	...
	SocketTopics.loadMoreUnreadTopics = function (socket, data, callback) {
		...
		var stop = start + Math.max(0, Math.min(meta.config.topicsPerPage, parseInt(data.topicsPerPage, 10) || meta.config.topicsPerPage));
