...
export default  function (Topics) {
	...
	async function getCidTids(params) {
		if (params.tags.length) {
			return _.intersection(...await Promise.all(params.tags.map(async (tag: string) => {
				const sets = params.cids.map((cid: string) => `cid:${cid}:tag:${tag}:topics`);
				return await db.getSortedSetRevRange(sets, 0, -1);
			})));
		}

		const sets : any[] = [];
		const pinnedSets : any[] = [];
	...
	async function sortTids(tids: string[], params) {
	...
	function floatPinned(topicData, sortFn: Function) {
	...
	async function filterTids(tids: string[], params) {
		const { filter } = params;
		const { uid } = params;
	...
	async function getTopics(tids: string[], params) {
	...
	Topics.calculateTopicIndices = function (topicData, start: number) {
		topicData.forEach((topic, index: number) => {
			if (topic) {
