...
export default  function (Topics) {
	...
	async function getTids(params) {
		...
		const tidsByFilter = { '': [], new: [], watched: [], unreplied: [] } as any;
		...
		tids_unread.forEach((t, i: number) => {
	...
	async function getFollowedTids(params) {
		...
		const filterCids = params.cid && params.cid.map((cid) => parseInt(cid, 10));
		if (filterCids) {
			const topicData = await Topics.getTopicsFields(tids, ['tid', 'cid']) as Array<any>;
			tids = topicData.filter((t) => filterCids.includes(t.cid)).map((t) => t.tid);
		}
		const scores = await db.sortedSetScores('topics:recent', tids);
		const data = tids.map((tid: string, index: number) => ({ value: String(tid), score: scores[index] }));
	...
	async function filterTidsThatHaveBlockedPosts(params) {
		...
		return await async.filter(params.tids, async (tid: string) => await doesTidHaveUnblockedUnreadPosts(tid, {
	...
	async function doesTidHaveUnblockedUnreadPosts(tid: string, params) {
	...
	Topics.pushUnreadCount = async function (uid: string) {
	...
	Topics.markAsUnreadForAll = async function (tid: string) {
		await Topics.markCategoryUnreadForAll(tid);
	};

	Topics.markAsRead = async function (tids: string[], uid: string) {
		if (!Array.isArray(tids) || !tids.length) {
			return false;
		}

		tids = _.uniq(tids).filter((tid: string) => tid && utils.isNumber(tid));
		...
		const topics = topicScores.filter((t, i: number) => t.lastposttime && (!userScores[i] || userScores[i] < t.lastposttime));
	...
	Topics.markAllRead = async function (uid: string) {
	...
	Topics.markTopicNotificationsRead = async function (tids: string[], uid: string) {
		if (!Array.isArray(tids) || !tids.length) {
			return;
		}
	...
	Topics.markCategoryUnreadForAll = async function (tid: string) {
		const cid = await Topics.getTopicField(tid, 'cid');
		await categories.markAsUnreadForAll(cid);
	};

	Topics.hasReadTopics = async function (tids: string[], uid: string) {
		...
		const cutoff = await Topics.unreadCutoff(uid);
		const result = tids.map((tid: string, index: number) => {
	...
	Topics.hasReadTopic = async function (tid: string, uid: string) {
		const hasRead = await Topics.hasReadTopics([tid], uid);
		return Array.isArray(hasRead) && hasRead.length ? hasRead[0] : false;
	};

	Topics.markUnread = async function (tid: string, uid: string) {
	...
	Topics.filterNewTids = async function (tids: string[], uid: string) {
		if (parseInt(uid, 10) <= 0) {
			return [];
		}
		const scores = await db.sortedSetScores(`uid:${uid}:tids_read`, tids);
		return tids.filter((tid: string, index: number) => tid && !scores[index]);
	};

	Topics.filterUnrepliedTids = async function (tids: string[]) {
		const scores = await db.sortedSetScores('topics:posts', tids);
		return tids.filter((tid: string, index: number) => tid && scores[index] !== null && scores[index] <= 1);
	};
};
