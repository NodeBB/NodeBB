...
export default  function (Topics) {
	Topics.createTags = async function (tags, tid: string, timestamp: number) {
	...
	Topics.updateCategoryTagsCount = async function (cids: string[], tags: string[]) {
	...
	Topics.validateTags = async function (tags: string[], cid: string, uid: string, tid = null) {
		...
		if (!isPrivileged && systemTags.length && removedTags.length && removedTags.some((tag: string) => systemTags.includes(tag))) {
			throw new Error('[[error:cant-remove-system-tag]]');
		}
	};

	async function filterCategoryTags(tags: string[], cid: string) {
	...
	Topics.createEmptyTag = async function (tag: string) {
		if (!tag) {
		...
		const isMembers = await db.isMemberOfSortedSets(
			allCids.map((cid: string) => `cid:${cid}:tags`), tag
		);
		const bulkAdd = allCids.filter((cid: string, index: number) => !isMembers[index])
			.map((cid: string) => ([`cid:${cid}:tags`, 0, tag]));
	...
	async function renameTag(tag: string, newTagName: string) {
		...
		await batch.processSortedSet(`tag:${tag}:topics`, async (tids: string[]) => {
			...
			await db.sortedSetAddBulk(topicData.map(
				(t, index: number) => [`cid:${t.cid}:tag:${newTagName}:topics`, scores[index], t.tid]
			));
			await db.sortedSetRemove(cids.map((cid: string) => `cid:${cid}:tag:${tag}:topics`), tids);
	...
	Topics.getTagTids = async function (tag, start: number, stop: number) {
	...
	Topics.getTagTidsByCids = async function (tag: string, cids: string[], start: number, stop: number) {
	...
	Topics.getTagTopicCount = async function (tag: string, cids = []) {
		let count = 0;
		if (cids.length) {
			count = await db.sortedSetsCardSum(
				cids.map((cid: string) => `cid:${cid}:tag:${tag}:topics`)
			);
		} else {
	...
	Topics.deleteTags = async function (tags: string[]) {
		if (!Array.isArray(tags) || !tags.length) {
			return;
		}
		...
		await db.sortedSetRemove(cids.map((cid: string) => `cid:${cid}:tags`), tags);
	...
	async function removeTagsFromTopics(tags: string[]) {
		await async.eachLimit(tags, 50, async (tag: string) => {
			...
			await db.deleteObjectFields(
				tids.map((tid: string) => `topic:${tid}`),
				['tags'],
			);
		});
	}

	Topics.deleteTag = async function (tag: string) {
		await Topics.deleteTags([tag]);
	};

	Topics.getTags = async function (start: number, stop: number) {
		return await getFromSet('tags:topic:count', start, stop);
	};

	Topics.getCategoryTags = async function (cids: string[], start: number, stop: number) {
	...
	Topics.getCategoryTagsData = async function (cids: string[], start: number, stop: number) {
	...
	async function getFromSet(set: string[] | string, start: number, stop: number) {
		let tags;
	...
	Topics.getTopicTags = async function (tid: string) {
		const data = await Topics.getTopicsTags([tid]);
		return data && data[0];
	};

	Topics.getTopicsTags = async function (tids: string[]) {
	...
	Topics.addTags = async function (tags: string[], tids: string[]) {
	...
	Topics.removeTags = async function (tags: string[], tids: string[]) {
		const topicData = await Topics.getTopicsFields(tids, ['tid', 'cid', 'tags']);
	...
	Topics.updateTopicTags = async function (tid: string, tags) {
	...
	Topics.deleteTopicTags = async function (tid: string) {
		...
		const sets = tags.map((tag: string) => `tag:${tag}:topics`)
			.concat(tags.map((tag: string) => `cid:${cid}:tag:${tag}:topics`));
	...
	Topics.searchAndLoadTags = async function (data) {
		...
		tagData.forEach((tag, index: number) => {
	...
	Topics.getRelatedTopics = async function (topicData, uid: string) {
