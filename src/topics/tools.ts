...
export default  function (Topics) {
	...
	topicTools.purge = async function (tid: string, uid: string) {
		const topicData = await Topics.getTopicData(tid);
		if (!topicData) {
			throw new Error('[[error:no-topic]]');
		}
	...
	topicTools.setPinExpiry = async (tid: string, expiry: number | string, uid: string) => {
		if (isNaN(parseInt(expiry as string, 10)) || expiry <= Date.now()) {
			throw new Error('[[error:invalid-data]]');
		}
	...
	topicTools.checkPinExpiry = async (tids: (string | null)[]) => {
		...
		tids = await Promise.all(tids.map(async (tid, idx) => {
			if (expiry[idx] && parseInt(expiry[idx], 10) <= now) {
				await togglePin(tid as string, 'system', false);
				return null;
			}
	...
	topicTools.orderPinnedTopics = async function (uid: string, data) {
		...
		await db.sortedSetAdd(
			`cid:${cid}:tids:pinned`,
			pinnedTids.map((tid: string, index: number) => index),
			pinnedTids
		);
	};

	topicTools.move = async function (tid: string, data) {
