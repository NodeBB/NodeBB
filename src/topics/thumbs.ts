...
Thumbs.exists = async function (id: string, path: string) {
...
Thumbs.get = async function (tids: string[]) {
	...
	let response = thumbs.map((thumbSet, idx) => thumbSet.map((thumb: string) => ({
		id: tids[idx],
		name: (() => {
...
async function getThumbs(set: string) {
...
Thumbs.migrate = async function (uuid: string, id: string) {
...
Thumbs.delete = async function (id: string, relativePaths: (string | string[])) {
	const isDraft = validator.isUUID(String(id));
	const set = `${isDraft ? 'draft' : 'topic'}:${id}:thumbs`;
	...
	if (isDraft && toDelete.length) { // drafts only; post upload dissociation handles disk deletion for topics
		await Promise.all(toDelete.map(async (absolutePath: string) => file.delete(absolutePath)));
	}
	...
	if (toRemove.length && !isDraft) {
		...
		await Promise.all([
			...
			Promise.all(toRemove.map(async (relativePath: string) => posts.uploads.dissociate(mainPid, relativePath.slice(1)))),
		]);
	}
};

Thumbs.deleteAll = async (id: string) => {
	const isDraft = validator.isUUID(String(id));
	const set = `${isDraft ? 'draft' : 'topic'}:${id}:thumbs`;
