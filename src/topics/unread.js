  ...
  function filterWatchedTids(uid, tids, callback) {
    db.sortedSetScores('uid:' + uid + ':followed_tids', tids, function(err, scores) {
      ...
      tids = tids.filter(function(tid, index) {
        return tid && Boolean(scores[index]);
      });
      callback(null, tids);
    });
  }
      ...
      function(results, next) {
        ...
        tids = topics.filter(function(topic, index) {
          return topic && topic.cid &&
						(Boolean(results.isTopicsFollowed[index]) || ignoredCids.indexOf(topic.cid.toString()) === -1) &&
    ...
    async.parallel({
      ...
      var result = tids.map(function(tid, index) {
        return !results.tids_unread[index] &&
					...
					Boolean(results.userScores[index] && results.userScores[index] >= results.recentScores[index]));
      });
