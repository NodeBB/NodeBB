...
export default  function (Topics) {
	...
	Topics.getTopicPosts = async function (topicData, set, start: number, stop: number, uid: string, reverse: string) {
		if (!topicData) {
			return [];
		}
		...
		if (allPosts.length !== postData.length) {
			...
			allPosts.reverse().forEach((p, index: number) => {
	...
	async function addEventStartEnd(postData, set, reverse, topicData) {
		...
		postData.forEach((p, index: number) => {
	...
	Topics.addPostData = async function (postData, uid: string) {
		if (!Array.isArray(postData) || !postData.length) {
			return [];
		}
		const pids = postData.map(post =>post && post.pid);

		async function getPostUserData(field: string, method: Function) {
		...
		const [
			...
			getPostUserData('uid', async (uids: string[]) => await posts.getUserInfoForPosts(uids, uid)),
			getPostUserData('editor', async (uids: string[]) => await user.getUsersFields(uids, ['uid', 'username', 'userslug'])),
	...
	Topics.addParentPosts = async function (postData) {
		...
		parentPosts.forEach((post, i: number) => {
	...
	Topics.calculatePostIndices = function (posts: Array<any>, start: number) {
	...
	Topics.getLatestUndeletedPid = async function (tid: string) {
	...
	Topics.getLatestUndeletedReply = async function (tid: string) {
		let isDeleted = false;
		let index = 0;
		do {
	...
	Topics.addPostToTopic = async function (tid: string, postData) {
		const mainPid = await Topics.getTopicField(tid, 'mainPid');
	...
	Topics.removePostFromTopic = async function (tid: string, postData) {
	...
	Topics.getPids = async function (tid: string) {
	...
	Topics.increasePostCount = async function (tid: string) {
		incrementFieldAndUpdateSortedSet(tid, 'postcount', 1, 'topics:posts');
	};

	Topics.decreasePostCount = async function (tid: string) {
		incrementFieldAndUpdateSortedSet(tid, 'postcount', -1, 'topics:posts');
	};

	Topics.increaseViewCount = async function (tid: string) {
		const cid = await Topics.getTopicField(tid, 'cid');
		incrementFieldAndUpdateSortedSet(tid, 'viewcount', 1, ['topics:views', `cid:${cid}:tids:views`]);
	};

	async function incrementFieldAndUpdateSortedSet(tid: string, field: string, by, set) {
	...
	Topics.getTitleByPid = async function (pid: string) {
		return await Topics.getTopicFieldByPid('title', pid);
	};

	Topics.getTopicFieldByPid = async function (field: string, pid: string) {
		const tid = await posts.getPostField(pid, 'tid');
		return await Topics.getTopicField(tid, field);
	};

	Topics.getTopicDataByPid = async function (pid: string) {
		const tid = await posts.getPostField(pid, 'tid');
		return await Topics.getTopicData(tid);
	};

	Topics.getPostCount = async function (tid: string) {
		return await db.getObjectField(`topic:${tid}`, 'postcount');
	};

	async function getPostReplies(pids: string[], callerUid: string) {
	...
	Topics.syncBacklinks = async (postData) => {
		...
		add = add.filter((_tid: string, idx: number) => topicsExist[idx] && !current.includes(_tid) && tid !== _tid);
		...
		await Promise.all(add.map(async (tid: number) => {
