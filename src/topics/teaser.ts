...
export default  function (Topics) {
	Topics.getTeasers = async function (topics, options) {
		...
		const counts : any[] = [];
		const teaserPids : any[] = [];
		const tidToPost = {} as any;
	...
	function calcTeaserIndex(teaserPost: string, postCountInTopic: number, sortNewToOld: boolean) {
		if (teaserPost === 'first') {
			return 1;
		}
	...
	function replaceImgWithAltText(str: string) {
		return String(str).replace(/<img .*?alt="(.*?)"[^>]*>/gi, '$1');
	}

	async function handleBlocks(uid: string, teasers: Array<any>) {
	...
	async function getPreviousNonBlockedPost(postData, blockedUids: (string | number)[]) {
		let isBlocked = false;
	...
	Topics.getTeasersByTids = async function (tids: string[], uid: string) {
