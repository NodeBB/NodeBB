...
Scheduled.handleExpired = async function () {
	...
	const uids = _.uniq(topicsData.map((topicData) => topicData.uid)).filter((uid: string) => uid); // Filter guests topics
...
Scheduled.pin = async function (tid: string, topicData) {
	return Promise.all([
...
function unpin(tid: string, topicData) {
	return [
...
async function sendNotifications(uids: string[], topicsData) {
	...
	postsData.forEach((postData, idx: number) => {
		postData.user  = {} as any;
		postData.user.username = uidToUsername[postData.uid];
		postData.topic = topicsData[idx];
	});

	return Promise.all(topicsData.map(
		(t, idx: number) => user.notifications.sendTopicNotificationToFollowers(t.uid, t, postsData[idx])
	).concat(
		topicsData.map(
			(t, idx: number) => socketHelpers.notifyNew(t.uid, 'newTopic', { posts: [postsData[idx]], topic: t })
		)
	));
}

async function updateUserLastposttimes(uids: string[], topicsData) {
	const lastposttimes = (await user.getUsersFields(uids, ['lastposttime'])).map((u) => u.lastposttime);

	let tstampByUid = {} as any;
	...
	tstampByUid = Object.fromEntries(
		Object.entries(tstampByUid).map(uidTimestamp => [uidTimestamp[0], Math.max(...uidTimestamp[1] as any)])
	);
