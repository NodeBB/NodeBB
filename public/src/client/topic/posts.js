...
define('forum/topic/posts', [
  ...
  Posts.onNewPost = function(data) {
    ...
    data.posts.forEach(function(post) {
      post.selfPost = Boolean(app.user.uid) && parseInt(post.uid, 10) === parseInt(app.user.uid, 10);
  ...
  Posts.showBottomPostBar = function() {
    ...
    if (Boolean(mainPost.length) && posts.length > 1 && $('.post-bar').length < 2 && placeHolder.length) {
