...
describe('Post\'s', () => {
	...
	describe('delete/restore/purge', () => {
		...
		it('should delete a post', (done) => {
			socketPosts.delete({ uid: voterUid }, { pid: replyPid, tid }, (err) => {
				assert.ifError(err);
			...
			async.waterfall([
				...
				function (next) {
					helpers.loginUser('global mod', '123456', (err, _jar) => {
						...
						request(`${nconf.get('url')}/api/topic/${tid}`, { jar, json: true }, (err, res, body) => {
							assert.ifError(err);
		...
		it('should restore a post', (done) => {
			socketPosts.restore({ uid: voterUid }, { pid: replyPid, tid }, (err) => {
				assert.ifError(err);
				posts.getPostField(replyPid, 'deleted', (err, isDeleted) => {
					assert.ifError(err);
		...
		it('should delete topic if last main post is deleted', (done) => {
			topics.post({ uid: voterUid, cid, title: 'test topic', content: 'test topic' }, (err, data) => {
				assert.ifError(err);
	...
	describe('edit', () => {
		...
		it('should error if title is too short', (done) => {
			socketPosts.edit({ uid: voterUid }, { pid, content: 'edited post content', title: 'a' }, (err) => {
		...
		it('should error if title is too long', (done) => {
			...
			socketPosts.edit({ uid: voterUid }, { pid, content: 'edited post content', title: longTitle }, (err) => {
		...
		it('should error with too few tags', (done) => {
			...
			socketPosts.edit({ uid: voterUid }, { pid, content: 'edited post content', tags: [] }, (err) => {
		...
		it('should error if content is too short', (done) => {
			socketPosts.edit({ uid: voterUid }, { pid, content: 'e' }, (err) => {
		...
		it('should error if content is too long', (done) => {
			...
			socketPosts.edit({ uid: voterUid }, { pid, content: longContent }, (err) => {
		...
		it('should disallow post editing for new users if post was made past the threshold for editing', (done) => {
			...
			setTimeout(() => {
				socketPosts.edit({ uid: voterUid }, { pid, content: 'edited post content again', title: 'edited title again', tags: ['edited-twice'] }, (err, data) => {
		...
		it('should edit a deleted post', (done) => {
			socketPosts.delete({ uid: voterUid }, { pid, tid }, (err) => {
				assert.ifError(err);
				socketPosts.edit({ uid: voterUid }, { pid, content: 'edited deleted content', title: 'edited deleted title', tags: ['deleted'] }, (err, data) => {
					assert.ifError(err);
	...
	describe('post queue', () => {
		...
		it('should add topic to post queue', (done) => {
			socketTopics.post({ uid }, { title: 'should be queued', content: 'queued topic content', cid }, (err, result) => {
				assert.ifError(err);
		...
		it('should add reply to post queue', (done) => {
			socketPosts.reply({ uid }, { content: 'this is a queued reply', tid: topicData.tid }, (err, result) => {
				assert.ifError(err);
				assert.strictEqual(result.queued, true);
				assert.equal(result.message, '[[success:post-queued]]');
				queueId = result.id;
		...
		it('should bypass post queue if user is in exempt group', (done) => {
			...
			socketTopics.post({ uid, emit: () => {} }, { title: 'should not be queued', content: 'topic content', cid }, (err, result) => {
				assert.ifError(err);
