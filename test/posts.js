...
describe('Post\'s', () => {
	...
	describe('Topic Linkbacks', () => {
		let tid1;
		before(async () => {
			tid1 = await topics.post({
				uid: 1,
				cid,
				title: 'Topic linkback testing - topic 1',
		...
		it('should create a topic event in the referenced topic', async () => {
			const topic = await topics.post({
				uid: 1,
				cid,
				title: 'Topic linkback testing - topic 2',
			...
			assert.strictEqual(events[0].type, 'linkback');
		...
		it('should not show linkback events if the feature is disabled', async () => {
			meta.config.topicLinkback = 0;

			await topics.post({
				uid: 1,
				cid,
				title: 'Topic linkback testing - topic 3',
				content: `Some text here for the OP &ndash; ${nconf.get('url')}/topic/${tid1}`,
			});
