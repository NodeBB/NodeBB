...
describe('Topic\'s', () => {
	...
	describe('unread', () => {
		...
		it('should mark topic notifications read', (done) => {
			async.waterfall([
				...
				function (next) {
					topics.reply({ uid, timestamp: Date.now(), content: 'some content', tid }, next);
				},
				function (data, next) {
	...
	describe('follow/unfollow', () => {
		...
		it('should error if not logged in', (done) => {
			socketTopics.changeWatching({ uid: 0 }, { tid, type: 'ignore' }, (err) => {
				assert.equal(err.message, '[[error:not-logged-in]]');
				done();
			});
		});
		...
		it('should filter ignoring uids', (done) => {
			socketTopics.changeWatching({ uid: followerUid }, { tid, type: 'ignore' }, (err) => {
				assert.ifError(err);
		...
		it('should error with invalid type', (done) => {
			socketTopics.changeWatching({ uid: followerUid }, { tid, type: 'derp' }, (err) => {
