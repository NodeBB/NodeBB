...
describe('socket.io', () => {
	...
	it('should toggle plugin install', function (done) {
		this.timeout(0);
		const oldValue = (process as any).env.NODE_ENV;
		...
		socketAdmin.plugins.toggleInstall({
			...
			(process as any).env.NODE_ENV = oldValue;
			done();
		});
	});
	...
	it('should get logs', (done) => {
		...
		import path from 'path';		meta.logs.path = path.join(nconf.get('base_dir'), 'test/files', 'output.log');
