...
describe('Controllers', () => {
	...
	it('should get post data', (done) => {
		request(`${nconf.get('url')}/api/post/pid/${pid}`, (err, res, body) => {
			assert.ifError(err);
			assert.equal(res.statusCode, 200);
			assert(body);
			done();
	...
	it('should get topic data', (done) => {
		request(`${nconf.get('url')}/api/topic/tid/${tid}`, (err, res, body) => {
			assert.ifError(err);
			assert.equal(res.statusCode, 200);
			assert(body);
			done();
	...
	it('should get category data', (done) => {
		request(`${nconf.get('url')}/api/category/cid/${cid}`, (err, res, body) => {
			assert.ifError(err);
			assert.equal(res.statusCode, 200);
			assert(body);
			done();
	...
	describe('account pages', () => {
		...
		it('should export users posts', (done) => {
			request(`${nconf.get('url')}/api/user/uid/foo/export/posts`, { jar }, (err, res, body) => {
				assert.ifError(err);
				assert.equal(res.statusCode, 200);
				assert(body);
				done();
		...
		it('should export users uploads', (done) => {
			request(`${nconf.get('url')}/api/user/uid/foo/export/uploads`, { jar }, (err, res, body) => {
				assert.ifError(err);
				assert.equal(res.statusCode, 200);
				assert(body);
				done();
		...
		it('should export users profile', (done) => {
			request(`${nconf.get('url')}/api/user/uid/foo/export/profile`, { jar }, (err, res, body) => {
				assert.ifError(err);
				assert.equal(res.statusCode, 200);
				assert(body);
				done();
		...
		it('should render edit/email', (done) => {
			request(`${nconf.get('url')}/api/user/foo/edit/email`, { jar, json: true }, (err, res, body) => {
				assert.ifError(err);
				assert.equal(res.statusCode, 200);
				done();
			});
	...
	describe('account follow page', () => {
		...
		before((done) => {
			user.create({ username: 'follower' }, (err, _uid) => {
				assert.ifError(err);
				uid = _uid;
				socketUser.follow({ uid }, { uid: fooUid }, (err) => {
					assert.ifError(err);
					socketUser.isFollowing({ uid }, { uid: fooUid }, (err, isFollowing) => {
						assert.ifError(err);
						assert(isFollowing);
						done();
					});
		...
		it('should return empty after unfollow', (done) => {
			socketUser.unfollow({ uid }, { uid: fooUid }, (err) => {
				assert.ifError(err);
				request(`${nconf.get('url')}/api/user/foo/followers`, { json: true }, (err, res, body) => {
					assert.ifError(err);
					assert.equal(res.statusCode, 200);
