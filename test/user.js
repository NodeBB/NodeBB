...
describe('User', () => {
	...
	describe('.search()', () => {
		...
		it('should filter users', (done) => {
			User.create({ username: 'ipsearch_filter' }, (err, uid) => {
				assert.ifError(err);
				User.setUserFields(uid, { banned: 1, flags: 10 }, (err) => {
					assert.ifError(err);
	...
	describe('profile methods', () => {
		...
		it('should return error if data is invalid', (done) => {
			socketUser.updateProfile({ uid }, null, (err) => {
				assert.equal(err.message, '[[error:invalid-data]]');
				done();
			});
		});
		...
		it('should return error if data is missing uid', (done) => {
			socketUser.updateProfile({ uid }, { username: 'bip', email: 'bop' }, (err) => {
				assert.equal(err.message, '[[error:invalid-data]]');
				done();
			});
		});
		...
		it('should update a user\'s profile', (done) => {
			User.create({ username: 'justforupdate', email: 'just@for.updated', password: '123456' }, (err, uid) => {
				...
				socketUser.updateProfile({ uid }, { ...data, password: '123456' }, (err, result) => {
					assert.ifError(err);
		...
		it('should change a user\'s password', (done) => {
			User.create({ username: 'changepassword', password: '123456' }, (err, uid) => {
				assert.ifError(err);
				socketUser.changePassword({ uid }, { uid, newPassword: '654321', currentPassword: '123456' }, (err) => {
					assert.ifError(err);
		...
		it('should not let user change another user\'s password', async () => {
			...
			let err;
			try {
				await socketUser.changePassword({ uid }, { uid: regularUserUid, newPassword: '654321', currentPassword: '123456' });
			} catch (_err) {
				err = _err;
			}
		...
		it('should not let user change admin\'s password', async () => {
			...
			let err;
			try {
				await socketUser.changePassword({ uid }, { uid: adminUid, newPassword: '654321', currentPassword: '123456' });
			} catch (_err) {
				err = _err;
			}
			assert.equal(err.message, '[[user:change_password_error_privileges]]');
		...
		it('should let admin change another users password', async () => {
			...
			await socketUser.changePassword({ uid: adminUid }, { uid, newPassword: '654321' });
		...
		it('should change username', (done) => {
			socketUser.changeUsernameEmail({ uid }, { uid, username: 'updatedAgain', password: '123456' }, (err) => {
				assert.ifError(err);
		...
		it('should not let setting an empty username', async () => {
			await socketUser.changeUsernameEmail({ uid }, { uid, username: '', password: '123456' });
		...
		it('should let updating profile if current username is above max length and it is not being changed', async () => {
			...
			await socketUser.changeUsernameEmail({ uid }, { uid, username: longName, email: 'verylong@name.com' });
		...
		it('should not update a user\'s username if a password is not supplied', async () => {
			let _err;
			try {
				await socketUser.updateProfile({ uid }, { uid, username: 'updatedAgain', password: '' });
			} catch (err) {
				_err = err;
			}
		...
		it('should change email', (done) => {
			User.create({ username: 'pooremailupdate', email: 'poor@update.me', password: '123456' }, (err, uid) => {
				assert.ifError(err);
				socketUser.changeUsernameEmail({ uid }, { uid, email: 'updatedAgain@me.com', password: '123456' }, (err) => {
					assert.ifError(err);
		...
		it('should update cover image', (done) => {
			...
			socketUser.updateCover({ uid }, { uid, imageData, position }, (err, result) => {
				assert.ifError(err);
				assert(result.url);
		...
		it('should upload cropped profile picture', (done) => {
			const imageData = 'data:image/png;base64,';
			socketUser.uploadCroppedPicture({ uid }, { uid, imageData }, (err, result) => {
				assert.ifError(err);
				assert(result.url);
		...
		it('should remove cover image', (done) => {
			socketUser.removeCover({ uid }, { uid }, (err) => {
				assert.ifError(err);
		...
		it('should change user picture', (done) => {
			socketUser.changePicture({ uid }, { type: 'default', uid }, (err) => {
				assert.ifError(err);
		...
		it('should fail to change user picture with invalid data', (done) => {
			socketUser.changePicture({ uid }, null, (err) => {
				assert.equal(err.message, '[[error:invalid-data]]');
				done();
			});
		});
		...
		it('should set user picture to uploaded', (done) => {
			User.setUserField(uid, 'uploadedpicture', '/test', (err) => {
				assert.ifError(err);
				socketUser.changePicture({ uid }, { type: 'uploaded', uid }, (err) => {
					assert.ifError(err);
					User.getUserField(uid, 'picture', (err, picture) => {
						assert.ifError(err);
		...
		it('should load edit/email page', (done) => {
			request(`${nconf.get('url')}/api/user/updatedagain/edit/email`, { jar, json: true }, (err, res, body) => {
				assert.ifError(err);
				assert.equal(res.statusCode, 200);
				assert(body);
				done();
		...
		it('should load user\'s groups page', (done) => {
			groups.create({
				...
				groups.join('Test', uid, (err) => {
					assert.ifError(err);
					request(`${nconf.get('url')}/api/user/updatedagain/groups`, { jar, json: true }, (err, res, body) => {
						assert.ifError(err);
						assert.equal(res.statusCode, 200);
	...
	describe('user info', () => {
		...
		it('should return the correct ban reason', (done) => {
			async.series([
				function (next) {
					User.bans.ban(testUid, 0, '', (err) => {
						assert.ifError(err);
						next(err);
					});
				},
				function (next) {
					User.getModerationHistory(testUid, (err, data) => {
						assert.ifError(err);
		...
		it('should ban user permanently', (done) => {
			User.bans.ban(testUid, (err) => {
				assert.ifError(err);
				User.bans.isBanned(testUid, (err, isBanned) => {
					assert.ifError(err);
		...
		it('should ban user temporarily', (done) => {
			User.bans.ban(testUid, Date.now() + 2000, (err) => {
				assert.ifError(err);
	...
	describe('socket methods', () => {
		...
		it('should delete user', async () => {
			...
			await socketUser.deleteAccount({ uid }, {});
		...
		it('should fail to delete user with wrong password', async () => {
			...
			let err;
			try {
				await socketUser.deleteAccount({ uid }, { password: '654321' });
			} catch (_err) {
				err = _err;
			}
		...
		it('should delete user with correct password', async () => {
			...
			await socketUser.deleteAccount({ uid }, { password: '123456' });
