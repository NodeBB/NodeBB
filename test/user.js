...
describe('User', function() {
  ...
  describe('.isReadyToPost()', function() {
    ...
    it('should allow a post if the last post time is > 10 seconds', function(done) {
      User.setUserField(testUid, 'lastposttime', Number(new Date()) - 11 * 1000, function() {
        Topics.post({
          uid: testUid,
          title: 'Topic 3',
          content: 'lorem ipsum',
    ...
    it('should error when a new user posts if the last post time is 10 < 30 seconds', function(done) {
      ...
      User.setUserField(testUid, 'lastposttime', Number(new Date()) - 20 * 1000, function() {
        Topics.post({
          uid: testUid,
          title: 'Topic 4',
          content: 'lorem ipsum',
    ...
    it('should not error if a non-newbie user posts if the last post time is 10 < 30 seconds', function(done) {
      User.setUserFields(testUid, {
        lastposttime: Number(new Date()) - 20 * 1000,
        reputation: 10
      }, function() {
        Topics.post({
          uid: testUid,
