...
describe('Messaging Library', () => {
	...
	before(async () => {
		...
		({ jar: mocks.users.foo.jar, csrf_token: mocks.users.foo.csrf } = await util.promisify(helpers.loginUser).default('foo', 'barbar'));
		({ jar: mocks.users.bar.jar, csrf_token: mocks.users.bar.csrf } = await util.promisify(helpers.loginUser).default('bar', 'bazbaz'));
	...
	describe('rooms', () => {
		...
		it('should create a new chat room', async () => {
			...
			await util.promisify(socketModules.chats.canMessage).default({ uid: mocks.users.foo.uid }, roomId);
		});
		...
		it('should change owner when owner leaves room', async () => {
			...
			await util.promisify(socketModules.chats.leave).default({ uid: mocks.users.herp.uid }, body.response.roomId);
		...
		it('should change owner if owner is deleted', async () => {
			...
			const { jar: senderJar, csrf_token: senderCsrf } = await util.promisify(helpers.loginUser).default('deleted_chat_user', 'barbar');
		...
		it('should send a message to a room', async () => {
			...
			const raw =
				await util.promisify(socketModules.chats.getRaw).default({ uid: mocks.users.foo.uid }, { mid: messageData.mid });
		...
		it('should return not allowed error if mid is not in room', async () => {
			...
			try {
				await util.promisify(socketModules.chats.getRaw).default({ uid: mocks.users.baz.uid }, { mid: 200 });
			} catch (err: any) {
				assert(err);
			...
			const raw = await util.promisify(socketModules.chats.getRaw).default({ uid: mocks.users.foo.uid }, { mid: message.mid });
		...
		it('should escape teaser', async () => {
			...
			const data = await util.promisify(socketModules.chats.getRecentChats).default(
				{ uid: mocks.users.foo.uid },
				{ after: 0, uid: mocks.users.foo.uid }
			);
	...
	describe('logged in chat controller', () => {
		...
		it('should return 404 if user is not in room', async () => {
			const data = await util.promisify(helpers.loginUser).default('baz', 'quuxquux');
