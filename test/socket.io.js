...
describe('socket.io', () => {
	...
	it('should post a topic', (done) => {
		io.emit('topics.post', { title: 'test topic title', content: 'test topic main post content', uid: adminUid, cid }, (err, result) => {
			assert.ifError(err);
	...
	it('should reply to topic', (done) => {
		io.emit('posts.reply', { tid, uid: adminUid, content: 'test post content' }, (err, result) => {
			assert.ifError(err);
