...
describe('Messaging Library', () => {
	...
	describe('rooms', () => {
		...
		it('should send a user-join system message when a chat room is created', (done) => {
			socketModules.chats.getMessages({ uid: fooUid }, { uid: fooUid, roomId, start: 0 }, (err, messages) => {
				assert.ifError(err);
				assert.equal(messages.length, 2);
				assert.strictEqual(messages[0].system, true);
				assert.strictEqual(messages[0].content, 'user-join');
				socketModules.chats.edit({ uid: fooUid }, { roomId, mid: messages[0].messageId, message: 'test' }, (err) => {
		...
		it('should fail to add user to room with invalid data', (done) => {
			socketModules.chats.addUserToRoom({ uid: fooUid }, null, (err) => {
				...
				socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId: null }, (err) => {
					assert.equal(err.message, '[[error:invalid-data]]');
					socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: null }, (err) => {
						assert.equal(err.message, '[[error:invalid-data]]');
						done();
					});
				});
		...
		it('should add a user to room', (done) => {
			socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: 'herp' }, (err) => {
				assert.ifError(err);
		...
		it('should get users in room', async () => {
			const data = await socketModules.chats.getUsersInRoom({ uid: fooUid }, { roomId });
			assert(Array.isArray(data) && data.length === 3);
		});

		it('should throw error if user is not in room', async () => {
			try {
				const data = await socketModules.chats.getUsersInRoom({ uid: 123123123 }, { roomId });
			} catch (err) {
		...
		it('should fail to add users to room if max is reached', (done) => {
			...
			socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: 'test' }, (err) => {
		...
		it('should fail to add users to room if user does not exist', (done) => {
			socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: 'doesnotexist' }, (err) => {
		...
		it('should fail to add self to room', (done) => {
			socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: 'foo' }, (err) => {
		...
		it('should send not a user-leave system message when a user tries to leave a room they are not in', async () => {
			...
			const messages = await socketModules.chats.getMessages({ uid: fooUid }, { uid: fooUid, roomId, start: 0 });
			assert.equal(messages.length, 4);
			const message = messages.pop();
			assert.strictEqual(message.system, true);
			assert.strictEqual(message.content, 'user-leave');
		...
		it('should change owner when owner leaves room', (done) => {
			socketModules.chats.newRoom({ uid: herpUid }, { touid: fooUid }, (err, roomId) => {
				assert.ifError(err);
				socketModules.chats.addUserToRoom({ uid: herpUid }, { roomId, username: 'baz' }, (err) => {
					assert.ifError(err);
		...
		it('should fail to remove user from room if user does not exist', (done) => {
			socketModules.chats.removeUserFromRoom({ uid: fooUid }, { roomId, uid: 99 }, (err) => {
		...
		it('should remove user from room', (done) => {
			socketModules.chats.newRoom({ uid: fooUid }, { touid: herpUid }, (err, roomId) => {
				...
				Messaging.isUserInRoom(herpUid, roomId, (err, isInRoom) => {
					assert.ifError(err);
					assert(isInRoom);
					socketModules.chats.removeUserFromRoom({ uid: fooUid }, { roomId, uid: herpUid }, (err) => {
						assert.equal(err.message, '[[error:cant-remove-last-user]]');
						socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: 'baz' }, (err) => {
							assert.ifError(err);
							socketModules.chats.removeUserFromRoom({ uid: fooUid }, { roomId, uid: herpUid }, (err) => {
								assert.ifError(err);
								Messaging.isUserInRoom(herpUid, roomId, (err, isInRoom) => {
		...
		it('should fail to send chat if content is empty', (done) => {
			socketModules.chats.send({ uid: fooUid }, { roomId, message: ' ' }, (err) => {
		...
		it('should send a message to a room', (done) => {
			socketModules.chats.send({ uid: fooUid }, { roomId, message: 'first chat message' }, (err, messageData) => {
				assert.ifError(err);
				assert(messageData);
		...
		it('should fail to send second message due to rate limit', (done) => {
			...
			const oldValue = meta.config.chatMessageDelay;
			meta.config.chatMessageDelay = 1000;
			socketModules.chats.send(socketMock, { roomId, message: 'first chat message' }, (err) => {
				assert.ifError(err);
				socketModules.chats.send(socketMock, { roomId, message: 'first chat message' }, (err) => {
					assert.equal(err.message, '[[error:too-many-messages]]');
		...
		it('should notify offline users of message', (done) => {
			...
			db.sortedSetAdd('users:online', Date.now() - ((meta.config.onlineCutoff * 60000) + 50000), herpUid, (err) => {
				assert.ifError(err);
				socketModules.chats.send({ uid: fooUid }, { roomId, message: 'second chat message' }, (err) => {
					assert.ifError(err);
					setTimeout(() => {
		...
		it('should fail to rename room with invalid data', (done) => {
			socketModules.chats.renameRoom({ uid: fooUid }, null, (err) => {
				...
				socketModules.chats.renameRoom({ uid: fooUid }, { roomId: null }, (err) => {
					assert.equal(err.message, '[[error:invalid-data]]');
					socketModules.chats.renameRoom({ uid: fooUid }, { roomId, newName: null }, (err) => {
						assert.equal(err.message, '[[error:invalid-data]]');
						done();
					});
				});
		...
		it('should rename room', (done) => {
			socketModules.chats.renameRoom({ uid: fooUid }, { roomId, newName: 'new room name' }, (err) => {
				assert.ifError(err);
				done();
			});
		});
		...
		it('should fail to load room if user is not in', (done) => {
			socketModules.chats.loadRoom({ uid: 0 }, { roomId }, (err) => {
				assert.equal(err.message, '[[error:no-privileges]]');
				done();
			});
		});
		...
		it('should load chat room', (done) => {
			socketModules.chats.loadRoom({ uid: fooUid }, { roomId }, (err, data) => {
				assert.ifError(err);
				assert(data);
		...
		it('should escape teaser', (done) => {
			socketModules.chats.send({ uid: fooUid }, { roomId, message: '<svg/onload=alert(document.location);' }, (err) => {
				assert.ifError(err);
				socketModules.chats.getRecentChats({ uid: fooUid }, { after: 0, uid: fooUid }, (err, data) => {
					assert.ifError(err);
	...
	describe('edit/delete', () => {
		...
		let mid2;
		before(async () => {
			await socketModules.chats.addUserToRoom({ uid: fooUid }, { roomId, username: 'baz' });
			mid = (await socketModules.chats.send({ uid: fooUid }, { roomId, message: 'first chat message' })).mid;
			mid2 = (await socketModules.chats.send({ uid: bazUid }, { roomId, message: 'second chat message' })).mid;
		});
		...
		it('should fail to edit message if new content is empty string', (done) => {
			socketModules.chats.edit({ uid: fooUid }, { mid, roomId, message: ' ' }, (err) => {
				assert.equal(err.message, '[[error:invalid-chat-message]]');
				done();
			});
		});
		...
		it('should fail to edit message if not own message', (done) => {
			socketModules.chats.edit({ uid: herpUid }, { mid, roomId, message: 'message edited' }, (err) => {
				assert.equal(err.message, '[[error:cant-edit-chat-message]]');
				done();
			});
		});
		...
		it('should edit message', (done) => {
			socketModules.chats.edit({ uid: fooUid }, { mid, roomId, message: 'message edited' }, (err) => {
				assert.ifError(err);
				socketModules.chats.getRaw({ uid: fooUid }, { mid }, (err, raw) => {
					assert.ifError(err);
		...
		it('should fail to delete message if not owner', (done) => {
			socketModules.chats.delete({ uid: herpUid }, { messageId: mid, roomId }, (err) => {
		...
		it('should mark the message as deleted', (done) => {
			socketModules.chats.delete({ uid: fooUid }, { messageId: mid, roomId }, (err) => {
				assert.ifError(err);
		...
		it('should not show deleted message to other users', (done) => {
			socketModules.chats.getMessages({ uid: herpUid }, { uid: herpUid, roomId, start: 0 }, (err, messages) => {
				assert.ifError(err);
		...
		it('should restore the message', (done) => {
			socketModules.chats.restore({ uid: fooUid }, { messageId: mid, roomId }, (err) => {
				assert.ifError(err);
				db.getObjectField(`message:${mid}`, 'deleted', (err, value) => {
					assert.ifError(err);
		...
		describe('disabled via ACP', () => {
			...
			it('should error out for regular users', async () => {
				try {
					await socketModules.chats.delete({ uid: bazUid }, { messageId: mid2, roomId });
				} catch (err) {
			...
			it('should succeed for administrators', async () => {
				await socketModules.chats.delete({ uid: fooUid }, { messageId: mid2, roomId });
				await socketModules.chats.restore({ uid: fooUid }, { messageId: mid2, roomId });
			});
	...
	describe('logged in chat controller', () => {
		...
		it('should return chats page data', (done) => {
			request(`${nconf.get('url')}/api/user/herp/chats`, { json: true, jar }, (err, response, body) => {
				assert.ifError(err);
		...
		it('should return room data', (done) => {
			request(`${nconf.get('url')}/api/user/herp/chats/${roomId}`, { json: true, jar }, (err, response, body) => {
				assert.ifError(err);
				assert.equal(response.statusCode, 200);
		...
		it('should redirect to chats page', (done) => {
			request(`${nconf.get('url')}/api/chats`, { jar, json: true }, (err, res, body) => {
				assert.ifError(err);
		...
		it('should return 404 if user is not in room', (done) => {
			helpers.loginUser('baz', 'quuxquux', (err, jar) => {
				assert.ifError(err);
				request(`${nconf.get('url')}/api/user/baz/chats/${roomId}`, { json: true, jar }, (err, response) => {
					assert.ifError(err);
					assert.equal(response.statusCode, 404);
					done();
				});
